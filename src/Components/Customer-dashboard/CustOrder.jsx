...
const CustOrder = () => {
    ...
    const handleCancelOrder = async (item) => {
        const result = await Swal.fire({
            ...
            try {
                const response = await axios.post('http://localhost:5000/order/cancelOrder', {
                    orderId: item._id
                });
    ...
    const fetchOrders = async () => {
        try {
            const response = await axios.post('http://localhost:5000/order/getOrderByCustId/', {
    ...
    const handleSubmitFeedback = async (order) => {
        try {
            const response = await axios.post('http://localhost:5000/feedback/createFeedback', {
                orderId: order._id,
                custId: localStorage.getItem('id'),
